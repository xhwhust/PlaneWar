    ...
    QWidget(parent)
{
    ...
    bk1.load("..\\photo\\p1.jpg") ;//加载背景图片
    moneyPic.load("..\\photo\\money.png") ;//钱
    pPic1.load("..\\photo\\plane1.png") ;//加载我方飞机图片
    _pPic1.load("..\\photo\\enemy_1.png") ;//敌机1
    _pPic2.load("..\\photo\\enemy_2.png") ;//敌机2
    _pPic3.load("..\\photo\\enemy_3.png") ;
    _pPic4.load("..\\photo\\enemy_4.png") ;
    _pPic5.load("..\\photo\\enemy_6.png") ;//boss
    _pPic6.load("..\\photo\\enemy_5.png") ;
    bPic1.load("..\\photo\\bullet1.png") ;//加载我方子弹图片
    bPic3.load("..\\photo\\bullet4.png") ;
    bPic4.load("..\\photo\\bullet5.png") ;
    bPic5.load("..\\photo\\bullet_6.png") ;
...
void oneWidget::paintEvent(QPaintEvent *){
    QPainter p(this) ;
                        ...
                        if(isCrash(enemys[j],bullet)){ //判断是否打中敌方飞机
                            ...
                            if(enemys[j].life == 0){
                                if(qrand()%9 == 0){
                                    ...
                                    p.drawPixmap(QRect(QPoint(prop->x,prop->y),prop->size),prop->pic);
                                    props.append(*prop);
                                }else if(qrand()%15 == 0){//击毁飞机后产生道具散弹的概率
...
void oneWidget::mouseMoveEvent(QMouseEvent * ev){  //鼠标即飞机移动坐标获取
    plane->x = ev->pos().rx() ;
    plane->y = ev->pos().ry() ;
...
void oneWidget::mousePressEvent(QMouseEvent * ev){  //鼠标单击射击
...
void oneWidget::controlTime(){  //定时
    ...
    connect(timer10,SIGNAL(timeout()),this,SLOT(createEnemy()));//自动瞄准飞机
    timer10->start(intervalTime10);


    timer11= new QTimer(this) ;
    connect(timer11,SIGNAL(timeout()),this,SLOT(createEnemy()));
}
void oneWidget::move(){  //按照定时改变子弹，飞机坐标使其移动
    if(bullets.size() > 0){
        for(int i = 0 ; i < bullets.size() ; i++){
            if(bullets[i].type == 2){
...
void oneWidget::createEnemy(){  //按照时间生成敌机，根据信号发送者的不同创造不同敌机
    if((QTimer*)sender() == timer2){
        Enemy* enemy = new Enemy(qrand()%(WIDTH-80),70,QSize(80,80),0,2,1,_pPic1,0,2) ;
        if(enemys.size() > 0){
            for(int i = 0 ; i < enemys.size() ; i++){
        ...
        Enemy* enemy = new Enemy(qrand()%(WIDTH-80),70,QSize(80,80),0,2,2,_pPic2,0,3) ;
        if(enemys.size() > 0){
            for(int i = 0 ; i < enemys.size() ; i++){
                if(isCrash(*enemy,enemys[i])){
                    i = -1 ;
        ...
        Enemy* enemy = new Enemy(-80,WIDTH/2,QSize(80,80),0,7,3,_pPic3,6,0) ;
        enemys.append(*enemy);
        update();
    }else if((QTimer*)sender() == timer6){
        Enemy* enemy = new Enemy(qrand()%(WIDTH-80),70,QSize(80,80),0,2,4,_pPic4,0,0) ;//自爆机
        enemys.append(*enemy);
        update();
    }else if((QTimer*)sender() == timer10){
        Enemy* enemy = new Enemy(qrand()%(WIDTH-80),70,QSize(80,80),0,3,7,_pPic6,0,3) ;
        if(enemys.size() > 0){
            for(int i = 0 ; i < enemys.size() ; i++){
                if(isCrash(*enemy,enemys[i])){
                    i = -1 ;
    ...
    else if((QTimer*)sender() == timer11){//自爆机二号
            Enemy* enemy = new Enemy(qrand()%(WIDTH-80),70,QSize(80,80),0,2,4,_pPic4,0,0) ;//自爆机
            enemys.append(*enemy);
            update();
        }
}
void oneWidget::createBullet(){  //创造敌方子弹
    if(enemys.size() > 0){
        for(int i = 0 ; i < enemys.size() ; i++){
            Enemy enemy = enemys.at(i) ;
            if(enemy.type == 1){  //根据飞机类型不同创造不同的子弹
                Bullet* bullet = new Bullet(enemy.x + enemy.size.rwidth()/2 - 5,enemy.y + enemy.size.rheight(),QSize(10,40),0,1,1,_bPic1,0,9) ;
                enemyBullets.append(*bullet);
            }else if(enemy.type == 2){
                Bullet* bullet = new Bullet(enemy.x + enemy.size.rwidth()/2 - 20,enemy.y + enemy.size.rheight(),QSize(40,40),0,1,2,_bPic2,0,7) ;
                Bullet bullet2 = *bullet ;
                Bullet bullet3 = *bullet ;
                bullet2.dx = -2 ;
                bullet3.dx = 2 ;
                ...
                Bullet* bullet = new Bullet(enemy.x + enemy.size.rwidth()/2 - 20,enemy.y + enemy.size.rheight()/2 - 20,QSize(40,40),0,1,3,bPic4,0,0) ;
                ...
                bullet2.dy = 15 ; bullet2.dx=0;
                bullet3.dx = -4;   bullet3.dy = 9 ;
                bullet4.dx = -6  ;   bullet4.dy = 4 ;
                bullet5.dx = -7 ;   bullet5.dy = 0 ;
                bullet6.dx = -4 ;    bullet6.dy = -4 ;
                bullet7.dx = 0 ;   bullet7.dy = -6 ;
                bullet8.dx = 4 ;    bullet8.dy = -3 ;
                bullet9.dx = 7 ;   bullet9.dy = 0 ;
                bullet10.dx = 4 ;    bullet10.dy = 6 ;
                bullet11.dx = 2;   bullet11.dy = 7;
                enemyBullets.append(bullet2);
                enemyBullets.append(bullet3);
                ...
                Bullet* bullet = new Bullet(enemy.x + enemy.size.rwidth()/2 - 20,enemy.y + enemy.size.rheight()/2 - 20,QSize(40,40),0,1,1,_bPic1,0,0) ;
                bullet->dx=(plane->x-enemy.x)/30; //30是每次刷新的时间间隔
                bullet->dy=(plane->y-enemy.y)/30;
                enemyBullets.append(*bullet);
            }
        }
    }
}
bool oneWidget::isCrash(Flyer a, Flyer b){  //碰撞算法
...
void oneWidget::changeAttack(){
    plane->attack = 0 ;
}
void oneWidget::changeShield(){
    plane->shield = 0 ;
}
void oneWidget::keyPressEvent(QKeyEvent * ev){
...
void oneWidget::closeEvent(QCloseEvent *){
    if(plane->kill > player.maxScore){
        player.maxScore = plane->kill ;
    }
    player.money += plane->wealth ;
...
void oneWidget::createBoss(){
    ...
    Enemy* enemy = new Enemy(WIDTH/2 - 200,-200,QSize(400,200),0,80,5,_pPic5,0,4) ;
    enemys.append(*enemy);
    update();
}
void oneWidget::createBullet_A(){
    bossBulletFlag++ ;
    timer11->stop();//自爆机二号停止
    if(bossBulletFlag%2 == 0){
        Bullet* bullet1 = new Bullet(qrand()%WIDTH,230,QSize(40,40),0,1,5,bPic6,0,0) ;
        Bullet* bullet2 =new Bullet(qrand()%WIDTH,230,QSize(40,40),0,1,5,bPic6,0,0) ;
        Bullet* bullet3 =new Bullet(qrand()%WIDTH,230,QSize(40,40),0,1,5,bPic6,0,0) ;
        bullet1->dx=(plane->x-bullet1->x)/30;
        bullet1->dy=(plane->y-230)/30;
        bullet2->dx=(plane->x-bullet2->x)/30;
        bullet2->dy=(plane->y-230)/30;
        bullet3->dx=(plane->x-bullet3->x)/30;
        bullet3->dy=(plane->y-230)/30;
...
void oneWidget::createBullet_B(){
     timer11->start(intervalTime11);//自爆机二号开启
...
void oneWidget::getPlayer(Player player){
    this->player = player ;
    spFlag[0] = player.p1 ;
    spFlag[1] = player.p2 ;
    spFlag[2] = player.p3 ;
