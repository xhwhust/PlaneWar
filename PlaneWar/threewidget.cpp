...
ThreeWidget::ThreeWidget(QWidget *parent) : QWidget(parent)
{
    ...
    _pPic233.load("..\\photo\\enemy233.png") ;
    _pPic2333.load("..\\photo\\enemy2333.png") ;
...
void ThreeWidget::paintEvent(QPaintEvent *){
    QPainter p(this) ;
    ...
    if(plane->kill % 10 == 0 && flag[plane->kill] == 0){
        plane->skill ++ ;
        flag[plane->kill] = 1 ;
    }
    //死亡判定
...
void ThreeWidget::mouseMoveEvent(QMouseEvent * ev){  //鼠标即飞机移动坐标获取
    plane->x = ev->pos().rx() ;
    plane->y = ev->pos().ry() ;
    update();
}
void ThreeWidget::mousePressEvent(QMouseEvent * ev){  //鼠标单击射击
...
void ThreeWidget::controlTime(){  //定时
...
void ThreeWidget::move(){  //按照定时改变子弹，飞机坐标使其移动
    if(bullets.size() > 0){
        for(int i = 0 ; i < bullets.size() ; i++){
...
void ThreeWidget::createEnemy(){  //按照时间生成敌机，根据信号发送者的不同创造不同敌机
    if((QTimer*)sender() == timer2){
        ...
        Enemy* enemy = new Enemy(qrand()%(WIDTH-80),70,QSize(80,80),0,2,4,_pPic4,0,0) ;
        enemys.append(*enemy);
        update();
    }
    else if((QTimer*)sender() == timer233){
        Enemy* enemy = new Enemy(qrand()%(WIDTH-80),70,QSize(80,80),0,3,233,_pPic233,0,3) ;
        if(enemys.size() > 0){
            for(int i = 0 ; i < enemys.size() ; i++){
                if(isCrash(*enemy,enemys[i])){
                    i = -1 ;
        ...
        enemys.append(*enemy);
        update();
    }else if((QTimer*)sender() == timer2333){
        Enemy* enemy = new Enemy(qrand()%(WIDTH-80),70,QSize(80,80),0,3,2333,_pPic2333,0,3) ;
        if(enemys.size() > 0){
            for(int i = 0 ; i < enemys.size() ; i++){
                if(isCrash(*enemy,enemys[i])){
                    i = -1 ;
...
void ThreeWidget::createBullet(){  //创造敌方子弹
    if(enemys.size() > 0){
        for(int i = 0 ; i < enemys.size() ; i++){
            Enemy enemy = enemys.at(i) ;
...
bool ThreeWidget::isCrash(Flyer a, Flyer b){  //碰撞算法
...
void ThreeWidget::changeAttack(){
    plane->attack = 0 ;
}
void ThreeWidget::changeShield(){
    plane->shield = 0 ;
}
...
void ThreeWidget::closeEvent(QCloseEvent *){
    if(plane->kill > player.maxScore){
        player.maxScore = plane->kill ;
    }
    player.money += plane->wealth ;
...
void ThreeWidget::createBullet_A(){
    bossBulletFlag++ ;
...
void ThreeWidget::createBullet_B(){
...
void ThreeWidget::getPlayer(Player player){
    this->player = player ;
    spFlag[0] = player.p1 ;
    spFlag[1] = player.p2 ;
    spFlag[2] = player.p3 ;
